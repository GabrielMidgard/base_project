            ...
            <li class="" v-nav-toggle>
              <a href="#" class="search-toggle iq-waves-effect d-flex align-items-center bg-primary rounded">
                ...
                <div class="caption" style="width: 96px;">
                  <h6 class="mb-0 line-height text-white">{{userJsonx.username}}</h6>
              ...
              <div class="iq-sub-dropdown iq-user-dropdown">
                <div class="iq-card shadow-none m-0">
                  <div class="iq-card-body p-0 ">
                    <div class="bg-primary p-3">
                      <h5 class="mb-0 text-white line-height">Hello {{userJsonx.username}}</h5>
...
import SideBarItems from '../services/menu'
import profile from '../assets/images/user/user-1.jpeg'
import loader from '../assets/images/logos/logoazul@3x.png'//loader.gif
...
export default {
  ...
  methods: {
    getMenuSideBar(){
      ...
      var paramsData = {
        rol: elementData.fk_rol
      }
     // this.verticalMenu = SideBarItems.getMenuSideBar(paramsData);
      //this.verticalMenu = menuSideBar;
      
      SideBarItems.getMenuSideBar(paramsData)
        .then(response => {
          if (response.status == 200) {
            var data = response.data;
            this.verticalMenu = data;
          }else{
  ...
  beforeMount() {
    const userJson = localStorage.getItem('admin_up_user_info');
    if(userJson == null){
      localStorage.removeItem('admin_up_user_info')
      localStorage.removeItem('admin_up_access_token')
      this.$router.push({ name: 'auth.login' })
    }else{
      this.userJsonx = JSON.parse(userJson);
    }
